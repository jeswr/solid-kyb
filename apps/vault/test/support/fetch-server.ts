 ...
 * REAL local TCP listener, so the grant-rail tests exercise it through a genuine HTTP round
 * trip — `SolidTestAccount.authFetch` (real DPoP-bound requests) has no "give me the
 * headers only" seam, so this is what lets the suite drive the route with an actually-
 * transmitted DPoP proof rather than a hand-built `Request` object.
 */
