 ...
 * `@jeswr/solid-pod-guard`-fronted routes — mirrors the small-dollar-lending showcase's own
 * `createTestCallerIssuer` (jeswr/solid-lending, read-only reference; kept consistent
 * across the demo suites' test infrastructure rather than reinvented per app).
 *
 * `@kyb/test-kit`'s OWN bundled OIDC dev issuer (`startSolidServer({oidc: true})`) mints its
 * WebID claims as `http://localhost:<port>` (see `packages/test-kit/src/harness.ts`) —
 * `@jeswr/solid-api-auth` accepts an `http:` WebID ONLY on hostname `localhost`, NEVER
 * `127.0.0.1` (no loopback exception for that hostname). This helper's caller identity
 * instead claims an `https:` WebID (no loopback exception needed either way for `https:`),
 * which sidesteps that hostname restriction entirely — used ONLY for the caller side of
 * `@jeswr/solid-pod-guard` routes (L1 DPoP-bound auth + L2 pim:storage binding, both of
 * which pod-guard's own default loopback-mapped fetch dereferences fine under
 * `allowInsecureLoopback`); the ACTUAL pod IO (seeding, the engine's Mode-2 podFetch) runs
 * through `@kyb/test-kit`'s own native OIDC account, which `@jeswr/solid-server` verifies
 * directly.
 */
