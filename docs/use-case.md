...
kyb.ttl`): FIBO Business Entities (leaf modules under `BE/LegalEntities/` and `BE/OwnershipAndControl/`, e.g. `https://spec.edmcouncil.org/fibo/ontology/BE/LegalEntities/LEIEntities/`, all
...
|---|---|---|---|---|---|---|
| FIBO — BE domain (LegalEntities: LEIEntities, LegalPersons, FormalBusinessOrganizations; OwnershipAndControl: OwnershipParties, Executives) | OWL, RDF/XML + Turtle | Legal-entity, LEI, ownership, and officer/signatory classes | Joint copyright: EDM Council + Object Management Group | leaf modules under `BE/LegalEntities/` and `BE/OwnershipAndControl/` (e.g. `https://spec.edmcouncil.org/fibo/ontology/BE/LegalEntities/LEIEntities/`) — all fetched modules `Release` maturity, `200 text/turtle` (re-verified 2026-07-23; note the bare `.../ontology/BE/` domain root itself 404s — only the leaf ontology modules dereference) | Org-identity, beneficial-ownership, and officer-authorization credentials | Free; version-pinned, re-checked on FIBO quarterly releases |
