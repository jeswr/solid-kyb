/**
 * The dev/test Solid-server harness (sm-qz3j seeder round): server boot, account
 * provisioning, and pod seeding behind a server-agnostic interface. Ported from
 * solid-mortgage's `packages/solid-kit/src/harness.ts` (jeswr/solid-mortgage, read-only
 * reference) and trimmed to what this repo's three seeding call sites need (dev-pod
 * script, e2e global-setup, the seeder integration suite).
 *
 * Implementation: sparq's `@jeswr/solid-server` (the `sparq-lws-wasm` WASM distribution,
 * vendored — see docs/framework-deps.md), one in-memory pod instance per identity on
 * dynamically acquired ports.
 *
 * Two modes:
 ...
 *   fixture (discovery doc + JWKS + WebID doc + `at+jwt` minting) mirrors sparq's own
 *   `packages/solid-server/test/oidc.test.mjs` (sparq-org/sparq @ 947480b02fcbd174f80f9247b55ee02202d2e345).
 */
...
import { type ResourceFixture, seedPod } from "./seed.ts";
...
async function createDevIssuerIdentity(): Promise<{
  ...
  async function accessToken(): Promise<string> {
    ...
    const value = await new SignJWT({
      ...
      cnf: { jkt: thumbprint },
      webid,
      // Required by `@jeswr/solid-api-auth`'s RFC 9068 sanity check (a
      // present-but-empty/absent `sub` is rejected before `webid` is even
      // read) — every `@jeswr/solid-pod-guard`-fronted app route verifies
      // through that package.
      sub: webid,
    })
      // RFC 9068 `at+jwt`: strict resource-server verifiers
      // (`@jeswr/solid-api-auth`'s `DpopApiVerifier`) reject a token without
      // it — omitting this left every pod-guard-fronted route rejecting this
      // harness's tokens with a 401 ("unexpected typ JWT header value")
      // before reaching any app logic. sparq's own OIDC verification
      // (`@solid/access-token-verifier`, used for direct pod WAC reads) is
      // unaffected either way — this only widens what verifies, never narrows.
...
/**
 * Boot the workspace's dev/test Solid server. All three seeding call sites (dev-pod
 * script, e2e global-setup, the seeder integration suite) go through this entry point.
 */
